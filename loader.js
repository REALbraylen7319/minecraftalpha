...
*/
	...
	const fileUrl = `${currentProtocol}//${currentHost}${currentPathname}/game.gz`;
	IPFS_GATEWAYS.push(fileUrl);
	...
	async function tryDownloadClient(ipfsCID, ipfsPath) {
		updateProgressBar("Update: 0 / " + Math.round(opts.dlSize * 0.001) + " kB", fileUrl, 0.0);
		try {
			var j = await tryDownloadURL(fileUrl);
			if(j) {
				if(opts.gzip) {
					try {
						...
						updateProgressBar("Client decompress failed!", fileUrl, -1);
						console.error("Caught exception during decompress: " + fileUrl);
						console.error(ex);
					}
				}else {
					return j;
			...
			updateProgressBar("Client download failed!", fileUrl, 1.0);
			console.error("Caught exception during download: " + fileUrl);
			console.error(ex);
		}
		return null;
	}	
